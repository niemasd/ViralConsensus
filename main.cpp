...
int main(int argc, char** argv) {
    ...
    std::vector<std::pair<int32_t, int32_t>> min_max_primer_inds = find_overlapping_primers(ref.length(), user_args.primer_bed_fn, user_args.primer_offset);
    counts_t counts = compute_counts(user_args.in_reads_fn, ref, user_args.min_qual);
